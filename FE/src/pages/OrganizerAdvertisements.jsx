...
const emptyForm = { type: "EVENT", eventId: "", boothId: "", bannerFileId: null, adText: "", startAt: "", endAt: "" };
const statusLabel = {
...
export default function OrganizerAdvertisements() {
  ...
  const [pricing, setPricing] = useState({ eventAdPrice: 100000, boothAdPrice: 0, paymentExpiresInMinutes: 30 });
  ...
  useEffect(() => {
    ...
    setLoading(true); setError(""); setForm(emptyForm); setEditingId(null);
    Promise.all([eventApi.organizationList(organizationId, { size: 100 }), loadAds(organizationId)])
      .then(([eventResult]) => {
        const list = eventResult?.data?.content || [];
        setEvents(list);
        setForm((previous) => ({ ...previous, eventId: String(list[0]?.id || "") }));
      }).catch((e) => setError(e.message || "광고 정보를 불러오지 못했습니다."))
      .finally(() => setLoading(false));
  }, [organizationId]);

  useEffect(() => {
    if (form.type !== "BOOTH" || !form.eventId) { setBooths([]); return; }
    advertisementApi.boothCandidates(form.eventId)
      .then((result) => setBooths((result?.data || []).filter((booth) => String(booth.assignedOrganizationId) === String(organizationId))))
      .catch((e) => setError(e.message || "광고 신청 가능한 부스를 불러오지 못했습니다."));
  }, [form.type, form.eventId, organizationId]);
  ...
  const submit = async (event) => {
    ...
    try {
      const result = creativeOnly
        ...
        : form.type === "EVENT" ? await advertisementApi.createEvent(form.eventId, payload)
        : await advertisementApi.createBooth(form.boothId, payload);
      if (result?.data?.paymentOrder) {
        window.alert(`광고 신청이 저장되었습니다. 결제 주문 ${result.data.paymentOrder.orderNo}, 결제 금액 ${Number(result.data.paymentOrder.totalAmount).toLocaleString()}원`);
      }
      await loadAds(); reset();
  ...
  const edit = (ad) => {
    setEditingId(ad.id);
    setForm({ type: ad.eventId ? "EVENT" : "BOOTH", eventId: String(ad.eventId || ""), boothId: String(ad.boothId || ""),
  ...
  const cancel = async (id) => {
    ...
    try { await advertisementApi.cancel(id); await loadAds(); }
  ...
  return <><TopNav active="organizer" /><main className="min-h-screen bg-surface-container-low px-lg pb-xl pt-[76px]">
    <div className="max-w-[1100px] mx-auto space-y-lg">
      <header className="flex justify-between items-end"><div><p className="text-caption text-primary">ORGANIZER CENTER</p><h1 className="font-display-lg text-[32px]">광고 신청·관리</h1><p className="text-ink-muted">행사 광고는 결제 후, 부스 광고는 신청 즉시 플랫폼 심사를 받습니다.</p></div><Link to={`/organizer-admin?organizationId=${organizationId}`} className="text-caption">관리자 홈</Link></header>
      ...
      <section className="overflow-hidden bg-white border border-hairline rounded-2xl">
        <div className="p-lg bg-gradient-to-r from-primary/10 to-primary-container/10 flex flex-col md:flex-row md:items-center md:justify-between gap-md">
          ...
          <div className="flex items-center gap-sm text-caption text-ink-muted"><Icon name="schedule" /><span>결제 유효시간 {pricing.paymentExpiresInMinutes}분</span></div>
        </div>
        <div className="grid md:grid-cols-2">
          <div className="p-lg border-b md:border-b-0 md:border-r border-hairline">
            ...
            <p className="mt-lg text-[28px] font-bold text-on-surface">{Number(pricing.eventAdPrice).toLocaleString()}<span className="text-body font-normal text-ink-muted">원 / 건</span></p>
            <p className="mt-sm text-caption text-ink-muted">결제 완료 후 플랫폼 관리자 심사를 거쳐 노출됩니다.</p>
          </div>
          <div className="p-lg">
            <div className="flex items-start justify-between gap-md"><div className="flex gap-sm"><span className="w-10 h-10 rounded-xl bg-secondary/10 text-secondary grid place-items-center"><Icon name="storefront" /></span><div><p className="font-body-strong">부스 광고</p><p className="text-caption text-ink-muted">배정받은 부스를 행사 방문객에게 홍보</p></div></div><span className="text-caption px-sm py-xs rounded-full bg-secondary/10 text-secondary">무료</span></div>
            <p className="mt-lg text-[28px] font-bold text-on-surface">{Number(pricing.boothAdPrice).toLocaleString()}<span className="text-body font-normal text-ink-muted">원</span></p>
            <p className="mt-sm text-caption text-ink-muted">결제 없이 신청 즉시 플랫폼 관리자 심사가 시작됩니다.</p>
          </div>
        </div>
      </section>
      <section className="bg-white border border-hairline rounded-2xl p-lg space-y-lg">
        <div className="flex items-start justify-between gap-md"><div><h2 className="font-display-md text-[22px]">{editingId ? creativeOnly ? "노출 콘텐츠 수정" : "광고 신청 수정" : "새 광고 신청"}</h2>{creativeOnly && <p className="text-caption text-primary mt-xs">노출 일정과 대상은 유지되며 이미지와 문구만 바로 변경됩니다.</p>}</div>{!editingId && <span className="text-caption px-sm py-xs rounded-full bg-surface-container">예상 광고비 {form.type === "EVENT" ? Number(pricing.eventAdPrice).toLocaleString() : Number(pricing.boothAdPrice).toLocaleString()}원</span>}</div>
        ...
        <form onSubmit={submit} className="space-y-lg">
          {!editingId && <div className="grid md:grid-cols-2 gap-md"><label>광고 유형<select className={field} value={form.type} onChange={(e) => change("type", e.target.value)}><option value="EVENT">행사 광고(유료)</option><option value="BOOTH">부스 광고(무료)</option></select></label><label>행사<select required className={field} value={form.eventId} onChange={(e) => change("eventId", e.target.value)}>{events.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div>}
          {!editingId && form.type === "BOOTH" && <label>광고 대상 부스<select required className={field} value={form.boothId} onChange={(e) => change("boothId", e.target.value)}><option value="">부스를 선택하세요</option>{booths.map((item) => <option key={item.boothId} value={item.boothId}>{item.boothCode} · {item.displayName}</option>)}</select></label>}
