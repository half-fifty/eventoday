...
export default function AdvertisementPaymentResult({ failed = false }) {
  ...
  useEffect(() => {
    ...
    paymentApi.confirm({ paymentKey, orderId, amount })
      .then((result) => {
        const account = result?.data?.virtualAccount || null;
        setVirtualAccount(account);
        setState(account ? "waiting" : "success");
        setMessage(account ? "가상계좌가 발급되었습니다. 입금이 확인되면 광고 심사가 시작됩니다." : "광고비 결제가 완료되었습니다. 플랫폼 심사를 기다려 주세요.");
      })
      .catch((error) => { setState("failed"); setMessage(error.message || "결제 승인에 실패했습니다."); });
  }, [failed, paymentKey, orderId, amount, validAmount]);
  return <main className="min-h-screen bg-surface-container-low grid place-items-center p-lg"><section className="w-full max-w-lg bg-white border border-hairline rounded-2xl p-xl text-center space-y-lg"><div className={`text-5xl ${state === "success" ? "text-primary" : state === "failed" ? "text-error" : "text-ink-muted"}`}>{state === "success" ? "✓" : state === "failed" ? "!" : "…"}</div><h1 className="font-display-lg text-[28px]">{state === "confirming" ? "결제 확인 중" : state === "waiting" ? "입금 대기" : state === "success" ? "결제 완료" : "결제 실패"}</h1><p className="text-ink-muted">{message}</p>{virtualAccount && <div className="rounded-xl bg-surface-container p-md text-left"><p className="text-caption text-ink-muted">입금 계좌</p><p className="mt-xs font-body-strong">{virtualAccount.bankCode || "은행"} {virtualAccount.accountNumber}</p><p className="mt-xs text-caption text-ink-muted">예금주 {virtualAccount.customerName || "-"} · {Number(virtualAccount.amount || amount).toLocaleString()}원</p><p className="mt-xs text-caption text-error">입금기한 {virtualAccount.dueAt ? new Date(virtualAccount.dueAt).toLocaleString("ko-KR") : "발급 화면에서 확인"}</p></div>}<Link to="/organizer-admin/advertisements" className="inline-block px-xl py-sm bg-primary text-white rounded-full">광고 관리로 돌아가기</Link></section></main>;
}
